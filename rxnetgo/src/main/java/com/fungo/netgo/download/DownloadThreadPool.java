...
import com.fungo.netgo.task.PriorityBlockingQueue;
...
public class DownloadThreadPool {
    ...
    private NetExecutor executor;               //线程池执行器

    public NetExecutor getExecutor() {
        if (executor == null) {
            synchronized (DownloadThreadPool.class) {
                if (executor == null) {
                    executor = new NetExecutor(corePoolSize, MAX_POOL_SIZE, KEEP_ALIVE_TIME, UNIT, //
