...
public class HttpUtils {
    ...
    public static Request.Builder appendHeaders(Request.Builder builder, HttpHeaders headers) {
        ...
        try {
            for (Map.Entry<String, Object> entry : headers.getHeaderParams().entrySet()) {
...
//                String headerValue = URLEncoder.encode(entry.getValue(), "UTF-8");
    ...
    public static RequestBody generateMultipartRequestBody(HttpParams params, boolean isMultipart) {
        if (params.getFileParams().isEmpty() && !isMultipart) {
            ...
            //拼接文件
            for (Map.Entry<String, ArrayList<HttpParams.FileWrapper>> entry : params.getFileParams().entrySet()) {
    ...
    public static <T> CacheEntity<T> createCacheEntity(Headers responseHeaders, T data, CacheMode cacheMode, String cacheKey) {
        ...
        if (cacheMode == CacheMode.DEFAULT) {
            long date = HttpHeaders.Companion.getDate(responseHeaders.get(HttpHeaders.HEAD_KEY_DATE));
            long expires = HttpHeaders.Companion.getExpiration(responseHeaders.get(HttpHeaders.HEAD_KEY_EXPIRES));
            String cacheControl = HttpHeaders.Companion.getCacheControl(responseHeaders.get(HttpHeaders.HEAD_KEY_CACHE_CONTROL), responseHeaders.get(HttpHeaders.HEAD_KEY_PRAGMA));
    ...
    public static <T> void addCacheHeaders(com.fungo.netgo.request.base.Request request, CacheEntity<T> cacheEntity, CacheMode cacheMode) {
        ...
        if (cacheEntity != null && cacheMode == CacheMode.DEFAULT) {
            ...
            if (responseHeaders != null) {
                ...
                long lastModified = HttpHeaders.Companion.getLastModified(String.valueOf(eTag));
                if (lastModified > 0)
                    request.headers(HttpHeaders.HEAD_KEY_IF_MODIFIED_SINCE, HttpHeaders.Companion.formatMillisToGMT(lastModified));
            }
        }
    }
