...
object EncryptUtils {
    ...
    // 密钥
    private const val secretKey = "fungo@lx100$#365#$"
    // 向量
     ...
     * MD5加密Byte数据
     */
    fun encodeMD5(source: ByteArray): ByteArray {
        try {
    ...
    fun encodeMD5(source: String): String {
        return byte2HexStr(encodeMD5(source.toByteArray()))
    }
        ...
        try {// 优先使用第三方库
            ...
            try {
                if ("Dalvik" == vm) {// Android
                    ...
                    return String(decodeMethod!!.invoke(null, encodeSource.toByteArray(), 0) as ByteArray)
                } else { // JavaSE/JavaEE
                    clazz = Class.forName("sun.misc.BASE64Decoder")
                    decodeMethod = clazz!!.getMethod("decodeBuffer", String::class.java)
                    return String(decodeMethod!!.invoke(clazz.newInstance(), encodeSource) as ByteArray)
                }
            } catch (e1: ClassNotFoundException) {
                return null
            } catch (e1: Exception) {
